...
func (env *Env) Create(w http.ResponseWriter, r *http.Request) {
	...
	apiSubdomain := helpers.RandStringRunes(10)
	//apiSubdomain := fmt.Sprintf("%spid1", demoSubdomain)
	fmt.Println("gen subdomains", apiSubdomain)

	// Find ports
	port := helpers.GeneratePorts()
	...
	fmt.Println("apiSubdomain, apiPort", apiSubdomain, apiPort)
	...
	res, err := env.docker.CreateContainer(3, apiPort)
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println("res", res)

	_, err = fmt.Fprintf(w, "apiSubdomain: %s \n res: %s", apiSubdomain, res)
	if err != nil {
		fmt.Println(err)
	}
